...
export default function AdminHome() {
  ...
  const pendingOrders = orders.filter((o: Order) => o.status === 'pending_payment').length;
  ...
  const recentOrders = orders.filter((o: Order) => o.createdAt > sevenDaysAgo).length;
  const recentRevenue = orders
    .filter((o: Order) => o.status === 'paid' && o.createdAt > sevenDaysAgo)
    .reduce((sum: number, o: Order) => sum + o.total, 0);
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button 
                ...
                className="inline-flex items-center px-4 py-2 border border-slate-300 text-sm font-medium rounded-xl text-slate-700 bg-white/80 backdrop-blur-sm hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200"
              >
        ...
        <div className="bg-white/80 backdrop-blur-sm shadow-xl rounded-2xl border border-white/20 mb-8">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <button 
                onClick={() => window.open('/admin/inventory', '_blank')}
                className="group flex items-center p-4 border border-slate-200/50 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200 hover:shadow-lg"
              >
        ...
        <div className="bg-white/80 backdrop-blur-sm shadow-xl rounded-2xl border border-white/20">
          ...
          {orders.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button 
                onClick={() => window.open('/products', '_blank')}
                className="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                <PlusIcon className="w-4 h-4 mr-2" />
                Create Order
              </button>
                            ...
                            <button 
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  if (response.ok) {
                                    await fetchOrders(); // Refresh data instead of reloading page
                                  }
                                } catch (error) {
